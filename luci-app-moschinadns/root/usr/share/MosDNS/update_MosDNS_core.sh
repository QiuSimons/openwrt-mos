...
binpath=$(uci get MosDNS.MosDNS.binpath)
if [ -z "$binpath" ]; then
uci set MosDNS.MosDNS.binpath="/tmp/MosDNS/MosDNS"
binpath="/tmp/MosDNS/MosDNS"
fi
mkdir -p ${binpath%/*}
...
check_latest_version(){
	check_wgetcurl
	latest_ver="$($downloader - https://api.github.com/repos/IrineSistiana/mos-chinadns/releases/latest 2>/dev/null|grep -E 'tag_name' |grep -E 'v[0-9.]+' -o 2>/dev/null)"
		...
		doupdate_MosDNS_core
			...
			if [ ! -z "$upxflag" ]; then
				...
				if [ $filesize -gt 8000000 ]; then
					...
					doupx
					mkdir -p "/tmp/MosDNSupdate/MosDNS" >/dev/null 2>&1
					rm -fr /tmp/MosDNSupdate/MosDNS/${binpath##*/}
					/tmp/upx-${upx_latest_ver}-${Arch}_linux/upx $upxflag $binpath -o /tmp/MosDNSupdate/MosDNS/${binpath##*/}
					rm -rf /tmp/upx-${upx_latest_ver}-${Arch}_linux
					/etc/init.d/MosDNS stop nobackup
					rm $binpath
					mv -f /tmp/MosDNSupdate/MosDNS/${binpath##*/} $binpath
...
doupdate_MosDNS_core(){
	...
	*)
	...
	grep -v "^#" /usr/share/MosDNS/links.txt >/tmp/run/MCDlinks.txt
	while read link
	do
		eval link="$link"
		echo -e "try download: $link"
		$downloader /tmp/MosDNSupdate/${link##*/} "$link" 2>&1
		if [ "$?" != "0" ]; then
			echo "download failed try another download"
			rm -f /tmp/MosDNSupdate/${link##*/}
		else
			local success="1"
			break
		fi 
	done < "/tmp/run/MCDlinks.txt"
	rm /tmp/run/MCDlinks.txt
	[ -z "$success" ] && echo "no download success" && EXIT 1
	if [ "${link##*.}" == "zip" ]; then
		# unzip not tar
		# tar -zxf "/tmp/MosDNSupdate/${link##*/}" -C "/tmp/MosDNSupdate/"
		unzip "/tmp/MosDNSupdate/${link##*/}" -d "/tmp/MosDNSupdate/MosDNS"
		if [ ! -e "/tmp/MosDNSupdate/MosDNS/mos-chinadns" ]; then
			echo -e "Failed to download core."
			rm -rf "/tmp/MosDNSupdate" >/dev/null 2>&1
			EXIT 1
		else
			mv "/tmp/MosDNSupdate/MosDNS/mos-chinadns" "/tmp/MosDNSupdate/MosDNS/MosDNS"
		fi
		downloadbin="/tmp/MosDNSupdate/MosDNS/MosDNS"
	else
		downloadbin="/tmp/MosDNSupdate/${link##*/}"
	fi
	...
	/etc/init.d/MosDNS stop nobackup
	rm "$binpath"
	if [ -e "/tmp/MosDNSupdate/chn_ip.list" ]; then
		mv -f "/tmp/MosDNSupdate/chn_ip.list" "/usr/share/MosDNS"
	fi
	if [ -e "/tmp/MosDNSupdate/chn_domain.list" ]; then
		mv -f "/tmp/MosDNSupdate/chn_domain.list" "/usr/share/MosDNS"
	fi
	if [ -e "/tmp/MosDNSupdate/non_chn_domain.list" ]; then
		mv -f "/tmp/MosDNSupdate/non_chn_domain.list" "/usr/share/MosDNS"
	fi
	mv -f "$downloadbin" "$binpath"
	if [ "$?" == "1" ]; then
		echo "mv failed maybe not enough space please use upx or change bin to /tmp/MosDNS" 
		EXIT 1
	fi
	/etc/init.d/MosDNS start
	rm -rf "/tmp/MosDNSupdate" >/dev/null 2>&1

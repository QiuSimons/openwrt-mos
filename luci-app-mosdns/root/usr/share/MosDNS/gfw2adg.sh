...
nowmd5=${nowmd5%% *}
local lastmd5=$(uci get MosDNS.MosDNS.gfwlistmd5 2>/dev/null)
if [ "$nowmd5" != "$lastmd5" ]; then
	uci set MosDNS.MosDNS.gfwlistmd5="$nowmd5"
	uci commit MosDNS
	[ "$1" == "noreload" ] || /etc/init.d/MosDNS reload
fi
}
configpath=$(uci get MosDNS.MosDNS.configpath 2>/dev/null)
[ "$1" == "del" ] && sed -i '/programaddstart/,/programaddend/d' $configpath && checkmd5 "$2" && exit 0
gfwupstream=$(uci get MosDNS.MosDNS.gfwupstream 2>/dev/null)
